...
class ST3215(protocol_packet_handler):
    ...
    def __init__(
        ...
        if not self.portHandler.openPort():
            raise ValueError("Could not open UART")

        protocol_packet_handler.__init__(
            self,
            self.portHandler
        )
        ...
        self._position_sync_read = None
        self._position_sync_read_ids = None
    ...
    def ReadPositions(self, sts_ids):
        """
        Read the current position of multiple servos with one sync-read transaction.

        :param sts_ids: Iterable of servo IDs

        :return: Dict mapping servo ID to position, or None on communication failure.
        """
        ids = list(sts_ids)
        if not ids:
            return {}

        if self._position_sync_read is None:
            self._position_sync_read = GroupSyncRead(self, STS_PRESENT_POSITION_L, 2)

        sync_read = self._position_sync_read
        if self._position_sync_read_ids != tuple(ids):
            sync_read.clearParam()
            for sts_id in ids:
                sync_read.addParam(sts_id)
            self._position_sync_read_ids = tuple(ids)
        ...
        positions = {}
        for sts_id in ids:
            data = sync_read.data_dict.get(sts_id)
            if not data or len(data) < 3 or data[0] != 0:
                positions[sts_id] = None
                continue

            positions[sts_id] = self.sts_makeword(data[1], data[2])
